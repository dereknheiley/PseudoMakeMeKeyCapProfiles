...
//translate([0,0,0])mx_master_base(xu = 4.25, yu = 1 );  
...
//Column high sculpt 3 row system 
    ...
    [17.16,  17.16,   6.5, 	 6.5, 8.75,    0,   .5,     4,     0,     0,   2,   2,      1,      5,      1,    3.5,     2,       2], //R3 deepdish
      ...
      if(Stem == true){
         translate([0,0,StemBrimDep])rotate(stemRot)difference(){   
          union(){
            ...
            translate([0,0,-.001])skin([for (i=[0:stemLayers-1]) transform(translation(StemTranslation(i,keyID))*rotation(StemRotation(i, keyID)), elliptical_rectangle(StemTransform(i, keyID),b=[5.5,5.5], fn=8))]); //Transition Support for taller profilea
          }
          skin(StemCurve);
          skin(StemCurve2);
          if (Stab >= 2){
